    ...
    */
    ...
    public class Text : SettingContainer
    {
        ...
        public override int getHeight(float width)
        {
            ...
            Verse.Text.Anchor = t;
            int h = (int)Verse.Text.CalcHeight(Helpers.tryTranslate(text, tKey), width);
            Verse.Text.Font = GameFont.Small;
            Verse.Text.Anchor = TextAnchor.UpperLeft;
    ...
    public class RadioButtons : KeyedSettingContainer
    {
        ...
        public override void drawSetting(Listing_Standard listingStandard, string selectedMod)
        {
            ...
            foreach (XmlContainer option in buttons)
            {
                ...
                b = listingStandard.RadioButton_NewTemp(Helpers.tryTranslate(option.node["label"].InnerText, tKey), XmlMod.allSettings.dataDict[selectedMod+";" +key] == option.node["value"].InnerText, 0, Helpers.tryTranslate(str, tKeyTip));
    ...
    public class ScrollView : SettingContainer
    {
        ...
        public override void drawSetting(Listing_Standard listingStandard, string selectedMod)
        {
            ...
            Listing_Standard listing = new Listing_Standard();
            Rect scrollRect = new Rect(0, 0, baseRect.width - 20f, calcHeight(settings, baseRect.width - 20f));
            listing.BeginScrollView(baseRect, ref scrollPos, ref scrollRect);
            listing.verticalSpacing = listingStandard.verticalSpacing;
            foreach (SettingContainer setting in settings)
            {
                setting.drawSetting(listing, selectedMod);
            }
            listing.EndScrollView(ref scrollRect);
